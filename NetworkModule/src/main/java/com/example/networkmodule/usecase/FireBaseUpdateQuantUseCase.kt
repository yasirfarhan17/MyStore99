...
class FireBaseUpdateQuantUseCase @Inject constructor(
    private val firebaseDatabaseRepository: FirebaseDatabaseRepository
) {
    operator fun invoke(price: String,
                        id: String,
                        quant: String): Flow<Resource<String>> = channelFlow {
        send(Resource.Loading())
        firebaseDatabaseRepository.updateCart(price=price,id = id,quant=quant).collectLatest {
            if (it.isSuccess) {
